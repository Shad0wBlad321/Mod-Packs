...
namespace Casevac
{
		...
		public static void Postfix(Vector3 clickPos, Pawn pawn, ref List<FloatMenuOption> opts)
		{
			if (pawn.health.capacities.CapableOf(PawnCapacityDefOf.Manipulation))
			{
				foreach (LocalTargetInfo item11 in GenUI.TargetsAt(clickPos, TargetingParameters.ForRescue(pawn), thingsOnly: true))
				{
					...
					if (victim3.RaceProps.IsFlesh && !victim3.InBed() && pawn.CanReserveAndReach(victim3, PathEndMode.OnCell, Danger.Deadly, 1, -1, null, ignoreOtherReservations: true))
					{
						if ((!victim3.InMentalState || victim3.health.hediffSet.HasHediff(HediffDefOf.Scaria)))
						{
							opts.Add(FloatMenuUtility.DecoratePrioritizedTask(new FloatMenuOption("RH.CASEVAC".Translate(victim3.Named("PAWN")), delegate
							{
								var guestStatus = victim3.IsPrisonerOfColony || victim3.HostileTo(pawn) ? GuestStatus.Prisoner : GuestStatus.Guest;
								Building_Bed building_Bed2 = RestUtility.FindBedFor(victim3, pawn, true, guestStatus: guestStatus);
								if (building_Bed2 == null)
								{
									building_Bed2 = RestUtility.FindBedFor(victim3, pawn, false, false, guestStatus);
								}
								if (building_Bed2 == null)
								{
	...
	public class Pawn_PathFollower_StopDead
	{
		...
		{
			if (___pawn.jobs?.curDriver is JobDriver_CasevacRescue)
			{
				if (___pawn.CurJob.targetA.Thing.ParentHolder is Pawn_CarryTracker pawn_CarryTracker
						&& pawn_CarryTracker.pawn != ___pawn && (!lastPathTicks.TryGetValue(___pawn, out var value) || Find.TickManager.TicksGame != value))
				{
					var nodeLeftCount = pawn_CarryTracker.pawn.pather.curPath.NodesLeftCount;
					var pos = nodeLeftCount - 1 > 0 ? pawn_CarryTracker.pawn.pather.curPath.NodesReversed[nodeLeftCount - 2] : pawn_CarryTracker.pawn.Position;
					...
					lastPathTicks[___pawn] = Find.TickManager.TicksGame;
					___pawn.pather.StartPath(pos, PathEndMode.OnCell);
					return false;
				}
			}
			return true;

...
namespace BillDoorsFramework
{
    public class PenetratingBullet : Bullet
    {
        ...
        public override void Launch(Thing launcher, Vector3 origin, LocalTargetInfo usedTarget, LocalTargetInfo intendedTarget, ProjectileHitFlags hitFlags, bool preventFriendlyFire = false, Thing equipment = null, ThingDef targetCoverDef = null)
        {
            ...
            initialVector = origin - usedTarget.CenterVector3;
            initialVector.y = 0;
            trueOrigin = origin;
        }
        ...
        {
            float remainingRange = equipmentDef.Verbs[0].range - (ExactPosition - trueOrigin).magnitude + ExtraRange;
            if (remainingRange <= 0)
            {
                canDestroyNow = true;
                Destroy();
            ...
            for (int i = 1; i < num; i++)
            {
                ...
                Vector3 vec3 = ExactPosition - intVec.ToVector3();
                vec3.y = 0;
                if (intVec != Position && Math.Abs(Vector3.Angle(initialVector, vec3)) < PostPenetrationDeviationAngle)
                {
                    affectedCells.Add(intVec);
                }
            }
            int randIndex = Rand.Range(0, affectedCells.Count - 1);
            ...
            else if (Map != null && Position.InBounds(Map))
            {
                ...
                IntVec3 targetCell = affectedCells[randIndex];
                if (targetCell.InBounds(Map)) affectedTargets = targetCell.GetThingList(Map).ToList();
                Thing target = null;
                if (affectedTargets.Count > 0)
                {
                    target = affectedTargets[Rand.Range(0, affectedTargets.Count - 1)];
                }
                usedTarget = target == null ? targetCell : new LocalTargetInfo(target);
        ...
        private static float GetPawnArmor(Pawn pawn)
        {
            ...
            if (num > 0.0001f)
            {
                foreach (BodyPartRecord bodyPartRecord in pawn.RaceProps.body.AllParts)
                {
                    ...
                    if (bodyPartRecord2.depth == BodyPartDepth.Outside && ((double)bodyPartRecord2.coverage >= 0.1 || bodyPartRecord2.def == BodyPartDefOf.Eye || bodyPartRecord2.def == BodyPartDefOf.Neck))
                    {

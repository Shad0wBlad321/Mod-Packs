...
namespace Fortification
{
    public class Bullet_ClusterBomb : Projectile
	{
		...
		public override void Tick()
		{
			...
			tick++;
			IntRange tDDistance = GetComp<CompClusterBomb>().Props.TDDistance;
		...
		private void GoodwillCorrection(List<IntVec3> cells)
		{
			for (int i = 0; i < cells.Count; i++)
			{
				...
				{
					if (item is Pawn pawn && !pawn.Dead && !pawn.Faction.IsPlayer && pawn.Faction != null && !pawn.Faction.HostileTo(launcher.Faction))
					{
		...
		private void Dismantling()
		{
			...
			for (int i = 0; i < GetComp<CompClusterBomb>().Props.clusterBurstCount; i++)
			{
				...
				float errorRange = GetComp<CompClusterBomb>().Props.errorRange;
				List<IntVec3> list = (List<IntVec3>)GenRadial.RadialCellsAround(base.DestinationCell, errorRange, useCenter: true);

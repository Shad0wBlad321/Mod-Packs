...
namespace RedHorse
{
	public class Comp_MusicPlayer : ThingComp
	{
		...
		private void SwitchTracks()
		{
			...
			{
				Log.Error("Could not resolve next track.", ignoreStopLoggingLimit: false);
				return;
			}
			NextTune = def;
		...
		private bool TryResolveNextTrack(out TuneDef def)
		{
			...
			{
				Log.Error("Unable to create playlist!", ignoreStopLoggingLimit: false);
				return false;
			}
			TuneDef result = null;
			for (int i = 0; i < 999; i++)
		...
		private void DoTickerWork(int tickerAmount)
		{
			...
			if (isRadio && rareTickWorker <= 0)
			{
				...
				{
					Log.Error("Radio Error: Cannot resolve power trader comp.", ignoreStopLoggingLimit: false);
					return;
				}
				if (!powerTrader.PowerOn)
				{

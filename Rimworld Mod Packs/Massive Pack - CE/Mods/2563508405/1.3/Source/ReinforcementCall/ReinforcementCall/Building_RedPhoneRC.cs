...
namespace ReinforcementCall
{
	...
	public class Building_RedPhoneRC : Building
	{
		...
		public bool CanUseCommsNow
		{
			get
			{
				bool flag = base.Spawned && base.Map.gameConditionManager.ConditionIsActive(GameConditionDefOf.SolarFlare);
				return !flag && this.powerComp.PowerOn;
			}
		}
		...
		{
			bool flag = !ReachabilityUtility.CanReach(myPawn, this, PathEndMode.InteractionCell, Danger.Some, false, TraverseMode.ByPawn);
			...
			{
				bool flag2 = base.Spawned && base.Map.gameConditionManager.ConditionIsActive(GameConditionDefOf.SolarFlare);
				if (flag2)
				{
				...
				{
					bool flag3 = !this.powerComp.PowerOn;
					if (flag3)
					{
					...
					else
					{
						...
						else
						{
							...
							{
								bool flag5 = !this.CanUseCommsNow;
								if (flag5)
								{
									Log.Error(((myPawn != null) ? myPawn.ToString() : null) + " could not use comm console for unknown reason.", false);
		...
		public override IEnumerable<FloatMenuOption> GetFloatMenuOptions(Pawn myPawn)
		{
			...
			else
			{
				...
				{
					FloatMenuOption option = this.CommFloatMenuOption(commTarget, myPawn);
		...
		public FloatMenuOption CommFloatMenuOption(Faction commTarget, Pawn negotiator)
		{
			...
			else
			{
				...
				bool flag = !this.LeaderIsAvailableToTalk(commTarget);
				if (flag)
				{

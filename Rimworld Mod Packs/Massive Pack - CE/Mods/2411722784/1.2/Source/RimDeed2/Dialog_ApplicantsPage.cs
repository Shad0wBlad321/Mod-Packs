...
namespace Rimdeed
{
	public class Dialog_ApplicantsPage : Dialog_NodeTree
	{
			...
			: base(startNode, radioMode)
		{
			...
			{
				Find.World.GetComponent<RimdeedManager>().RegisterNewRecruiter(curPawn, Find.CurrentMap);
		...
		[TweakValue("0Rimdeed", -100, 1000)] public static float TraitBoxHeight = 100;
		...
		[TweakValue("0Rimdeed", -100, 1000)] public static float IncapableOfHeight = 100;
		...
		public override void DoWindowContents(Rect rect)
		{
			...
			if (this.applicants.applicants.Any())
            {
				...
				GUI.DrawTexture(new Rect(pawnPortrait.center.x - PawnPortraitSize.x / 2f, pawnPortrait.yMin - 24f,
					PawnPortraitSize.x, PawnPortraitSize.y), PortraitsCache.Get(curPawn, PawnPortraitSize));
				...
				var incapableOfBoxOuter = new Rect(traitsBoxOuter.xMax + IncapableOfPosXOffset, traitsBoxOuter.y, IncapableOfWidth, IncapableOfHeight);
				DrawIncapableOf(incapableOfBoxOuter);

				var greetingBox = new Rect(rect.x + 14, traitsBoxOuter.yMax + 90, rect.width - 38, 170);

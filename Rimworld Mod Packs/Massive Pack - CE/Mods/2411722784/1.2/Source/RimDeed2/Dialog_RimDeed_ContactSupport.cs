...
namespace Rimdeed
{
	public class Dialog_RimDeed_ContactSupport : Dialog_NodeTree
	{
		...
		public Dialog_RimDeed_ContactSupport(DiaNode startNode, bool radioMode)
			: base(startNode, radioMode)
		{
			...
			sendComplaintOption.action = delegate ()
			{
				Find.WindowStack.TryRemove(this);
				DiaNode diaNode = new DiaNode("RimDeed");
				Find.WindowStack.Add(new Dialog_RimDeed_ContactSupport_NextPage(diaNode, false));
				Find.World.GetComponent<RimdeedManager>().complaintResponceTick = Find.TickManager.TicksGame;

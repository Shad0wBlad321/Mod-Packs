...
namespace RimDeed.Dialogs
{
	public class Dialog_ApplicantsPage : Dialog_NodeTree
	{
		...
		private readonly Applicants applicants;
		private readonly DiaOption acceptOption;
		private readonly DiaOption cancelOption;
		private readonly DiaOption exitOption;
		private readonly DiaOption xOption;
			...
			: base(startNode, radioMode)
		{
			...
			cancelOption = new DiaOption("RimDeed.Cancel".Translate())
			{
				...
				action = delegate ()
				{
					Letter letter = RimdeedManager.Instance.lettersWithApplicants.FirstOrDefault(x => x.Value == applicants).Key;
		...
		public override void DoWindowContents(Rect rect)
		{
			...
			Rect logo = new Rect(logoCornerPosX, logoCornerPosY, logoCornerWidth, logoCornerHeight);
			GUI.DrawTexture(logo, Textures.Rimdeed_corner);
			if (applicants.applicants.Any())
			{
				Rect pawnListBox = new Rect(rect.x + 10, logo.yMax + 15, PawnListBoxWidth, PawnListBoxHeight);
				DrawPawnList(pawnListBox);

				Rect pawnName = new Rect(logo.xMax + 30, logo.y + 10, 334, 40);
				DrawPawnLabel(pawnName);

				Text.Font = GameFont.Small;
				Text.Anchor = TextAnchor.UpperLeft;

				Rect pawnInfoBox = new Rect(pawnListBox.xMax + 10, pawnName.yMax + 15, 300, 120);
				DrawMenuSection(pawnInfoBox);

				Rect pawnGenderAgeInfo = new Rect(pawnInfoBox.x, pawnInfoBox.y, 130, 40);
				Label(pawnGenderAgeInfo.ContractedBy(5f), CurPawnInfo());
				...
				if (curPawn.Faction != null)
				{
					...
					GUI.color = color7;
					DrawHighlightIfMouseover(rect23);
					...
					GUI.color = color7;
					Label(new Rect(rect24.x + rect24.height + 5f, rect24.y, rect24.width - 10f, rect24.height), curPawn.Faction.Name);
				...
				Rect pawnBackstoryInfo = new Rect(pawnInfoBox.x, pawnGenderAgeInfo.yMax + 5, pawnInfoBox.width, 35);
				Label(pawnBackstoryInfo.ContractedBy(5f), childhoodInfo);
				...
				if (curPawn.story.adulthood != null)
				{
					...
					Rect pawnBackstoryAdulthoodInfo = new Rect(pawnBackstoryInfo.x, pawnBackstoryInfo.yMax, pawnInfoBox.width, 35);
					Label(pawnBackstoryAdulthoodInfo.ContractedBy(5f), adulthoodInfo);
				}

				Rect pawnPortrait = new Rect(pawnInfoBox.xMax + 15, pawnInfoBox.y, 100, pawnInfoBox.height);
				DrawMenuSection(pawnPortrait);

				pawnPortrait = pawnPortrait.ContractedBy(17f);

				Rand.PushState();
				Rand.Seed = curPawn.thingIDNumber;
				Rot4 rotation = Rand.Chance(0.1f) ? Rot4.Random : Rot4.South; // just a joke
				Rand.PopState();
				...
				Rect traitsBoxOuter = new Rect(pawnInfoBox.x, pawnInfoBox.yMax + TraitBoxPosYOffset, TraitBoxWidth, TraitBoxHeight);
				DrawTraits(traitsBoxOuter);

				Rect incapableOfBoxOuter = new Rect(traitsBoxOuter.xMax + IncapableOfPosXOffset, traitsBoxOuter.y, IncapableOfWidth, traitsBoxOuter.height);
				DrawIncapableOf(incapableOfBoxOuter);

				Rect greetingBox = new Rect(rect.x + 14, traitsBoxOuter.yMax + 30, rect.width - 38, 180);
				DrawMenuSection(greetingBox);
				Rect greetingTextBox = greetingBox.ContractedBy(10f);
				greetingTextBox.height = 70;
				Rand.PushState(curPawn.thingIDNumber);

				string greetingText = applicants.greetings[curPawn.thingIDNumber];
				Rand.PopState();
				Label(greetingTextBox, greetingText);
				Rect passionsInfoBox = new Rect(greetingTextBox.x, greetingTextBox.yMax + 5, greetingTextBox.width, 30);
				Label(passionsInfoBox, "RimDeed.VeryPassionateWith".Translate());
			...
			else
			{
				...
				Label(error, "RimDeed.Error".Translate());
			}
		...
		private void DrawSkills(Rect passionsInfoBox)
		{
			...
			List<SkillDef> skillsWithPassions = curPawn.skills.skills.Where(x => !x.TotallyDisabled).OrderByDescending(x => x.Level).Take(2).Select(x => x.def).ToList();
			for (int j = 0; j < skillsWithPassions.Count; j++)
			{
				SkillDef skillDef = skillsWithPassions[j];
				float y = j * 27f + offset.y;
		...
		{
			DrawMenuSection(traitsBoxOuter);
			Rect traitsBox = traitsBoxOuter.ContractedBy(5f);

			List<Trait> traits = curPawn.story.traits.allTraits;
			Text.Font = GameFont.Medium;
			float currentY2 = traitsBox.y;
			Label(new Rect(traitsBox.x, currentY2, 200f, 30f), "Traits".Translate());
			currentY2 += 30f;
			Text.Font = GameFont.Small;
			if (traits == null || traits.Count == 0)
			{
				...
				{
					DrawHighlight(rect12);
				}
				Label(rect12, "None".Translate());
			...
			else
			{
				GenUI.DrawElementStack(new Rect(traitsBox.x, currentY2, traitsBox.width - 5f, traitsBox.height / 4), 22f,
					curPawn.story.traits.allTraits, delegate (Rect r, Trait trait)
					{
						...
						Label(new Rect(r.x + 5f, r.y, r.width - 10f, r.height), trait.LabelCap);
						if (Mouse.IsOver(r))
						{
							TooltipHandler.TipRegion(tip: new TipSignal(() => trait.TipString(curPawn), (int)currentY2 * 37), rect: r);
						}
					}, (trait) => Text.CalcSize(trait.LabelCap).x + 10f);
			}
		}

		private void DrawIncapableOf(Rect incapableOfBoxOuter)
		{
			DrawMenuSection(incapableOfBoxOuter);
			Rect incapableOfBox = incapableOfBoxOuter.ContractedBy(5f);
			Text.Font = GameFont.Medium;
			float currentY3 = incapableOfBox.y;
			Label(new Rect(incapableOfBox.x, currentY3, 200f, 30f), "IncapableOf".Translate(curPawn));
			currentY3 += 30f;
			Text.Font = GameFont.Small;
			...
			float workTagWidthGetter(WorkTags tag) => Text.CalcSize(tag.LabelTranslated().CapitalizeFirst()).x + 10f;
			...
			if (disabledTags == WorkTags.None)
			{
				...
				{
					DrawHighlight(rect13);
				}
				Label(rect13, "None".Translate());
			...
			else
			{
				void drawer(Rect r, WorkTags tag)
				{
					...
					Label(new Rect(r.x + 5f, r.y, r.width - 10f, r.height), tag.LabelTranslated().CapitalizeFirst());
					if (Mouse.IsOver(r))
					{
				...
				{
					GenUI.DrawElementStackVertical(new Rect(incapableOfBox.x, currentY3, incapableOfBox.width - 5f, incapableOfBox.height / 4), 22f, disabledTagsList, drawer, workTagWidthGetter);
				}
				else
				{
					GenUI.DrawElementStack(new Rect(incapableOfBox.x, currentY3, incapableOfBox.width - 5f, incapableOfBox.height / 4), 22f, disabledTagsList, drawer, workTagWidthGetter, 5f);
				}
			}
		}
		...
		private static string GetWorkTypeDisabledCausedBy(Pawn pawn, WorkTags workTag)
		{
			...
			foreach (object item in workTypeDisableCauses)
			{
				...
				{
					stringBuilder.AppendLine("IncapableOfTooltipBackstory".Translate((item as BackstoryDef).TitleFor(pawn.gender)));
				}
		...
		private void DrawPawnList(Rect rect)
		{
			...
			for (int i = 0; i < applicants.applicants.Count; i++)
			{
				...
				DrawOptionBackground(rect3, curPawn == pawn);
				...
				Label(label: !(pawn.Name is NameTriple nameTriple) ? pawn.LabelShort : string.IsNullOrEmpty(nameTriple.Nick) ? nameTriple.First : nameTriple.Nick, rect: rect4.TopPart(0.5f).Rounded());
				if (Text.CalcSize(pawn.story.TitleCap).x > rect4.width)
				{
					Label(rect4.BottomPart(0.5f).Rounded(), pawn.story.TitleShortCap);
				}
				else
				{
					Label(rect4.BottomPart(0.5f).Rounded(), pawn.story.TitleCap);
				}
		...
		{
			if (curPawn.Name is NameTriple nameTriple)
			{
				DrawMenuSection(rect);
				Text.Font = GameFont.Medium;
				Text.Anchor = TextAnchor.MiddleCenter;
				Label(rect.ContractedBy(3), nameTriple.ToStringFull);
			}
		}

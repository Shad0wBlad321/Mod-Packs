...
namespace Rimdeed
{
	public class Dialog_RimDeed_ContactSupport : Dialog_NodeTree
	{
		...
		private DiaOption sendComplaintOption;

		private DiaOption cancelOption;
		private DiaOption xOption;
		private Pawn negotiator;

		public Dialog_RimDeed_ContactSupport(Pawn negotiator, DiaNode startNode, bool radioMode)
			: base(startNode, radioMode)
		{
			...
			sendComplaintOption = new DiaOption("RimDeed.SendComplaint".Translate());
			sendComplaintOption.dialog = this;
			sendComplaintOption.action = delegate ()
			{
				Find.WindowStack.TryRemove(this);
				DiaNode diaNode = new DiaNode("RimDeed");
				Find.WindowStack.Add(new Dialog_RimDeed_ContactSupport_NextPage(negotiator, diaNode, false));
				RimdeedManager.Instance.complaintResponceTick = Find.TickManager.TicksGame;
				IncidentParms parms = StorytellerUtility.DefaultParmsNow(IncidentCategoryDefOf.Misc, Find.CurrentMap);
				Find.Storyteller.incidentQueue.Add(RD_DefOf.RD_ComplaintResponse, Find.TickManager.TicksGame + (int)(GenDate.TicksPerDay * Rand.Range(1f, 2f)), parms);
			};
		...
		public override void DoWindowContents(Rect inRect)
		{
			...
			Widgets.Label(howCanWeHelpYou, "RimDeed.HowCanWeHelpYou".Translate());

			Rect sendComplaint = new Rect(howCanWeHelpYou.x - 120, howCanWeHelpYou.yMax, inRect.width - 30f, 999f);
			GUIHelper.OptOnGUI(sendComplaintOption, sendComplaint);

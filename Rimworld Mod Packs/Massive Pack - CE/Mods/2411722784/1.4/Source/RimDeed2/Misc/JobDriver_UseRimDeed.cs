		...
		public override IEnumerable<Toil> MakeNewToils()
		{
			...
			openComms.initAction = delegate
			{
				...
				if (((Building_CommsConsole)actor.jobs.curJob.GetTarget(TargetIndex.A).Thing).CanUseCommsNow)
				{
					...
					if (Find.TickManager.TicksGame > RimdeedManager.Instance.BannedUntil)
                    {
						Find.WindowStack.Add(new Dialog_RimDeed_MainPage(pawn, diaNode, false));
					}
					else
                    {
						Find.WindowStack.Add(new Dialog_RimDeed_BanPage(diaNode, false));
					}
				}
			};

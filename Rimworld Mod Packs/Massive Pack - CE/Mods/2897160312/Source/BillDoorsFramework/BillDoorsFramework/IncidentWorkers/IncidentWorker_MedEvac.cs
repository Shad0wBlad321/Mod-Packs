...
namespace BillDoorsFramework
{
    public class IncidentWorker_MedEvac : IncidentWorker_DropThings
    {
        ...
        Pawn findMostWoundedPawn(List<Pawn> pawns)
        {
            ...
            if (pawns.Count == 1)
            {
                anyoneWounded = pawns[0].health.hediffSet.HasTendableInjury();
            }
            else if (pawns.Any())
            {
                pawns.SortBy((Pawn p) =>
                {
                    ...
                    if (p.health.hediffSet.HasTendableInjury()) { anyoneWounded = true; return p.health.summaryHealth.SummaryHealthPercent; }
                    return 6666;
                });
            }
            if (anyoneWounded)
        ...
        protected override bool TryExecuteWorker(IncidentParms parms)
        {
            ...
            Pawn pawn = FindMostWoundedPawn(pawns, cells);
            if (pawn == null)
            {
        ...
        protected override void SpawnSkyfaller(IncidentParms parms, IntVec3 pos)
        {
            ...
            {
                evac.intendedPawn = intendedPawn;
            }
        }
    }
}

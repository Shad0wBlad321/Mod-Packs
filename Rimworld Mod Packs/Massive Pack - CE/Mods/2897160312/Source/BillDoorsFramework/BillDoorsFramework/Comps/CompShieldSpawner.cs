    ...
    public class CompShieldSpawner : ThingComp
    {
        ...
        public virtual void ImpectEffect(Vector3 drawPos, float amout)
        {
            ...
            if (amout >= Props.damageLimit)
            {
                ...
                SoundDefOf.EnergyShield_Broken.PlayOneShot(new TargetInfo(parent.Position, parent.Map, false));
        ...
        public override string CompInspectStringExtra()
        {
            ...
            stringBuilder.Append(string.Format(Props.textRestartTiming, ((float)(Props.restartTiming) / 60f).ToString("0.##")));
            if (restartTiming > 0)
            {
                stringBuilder.AppendLine();
                stringBuilder.Append(string.Format(Props.textRestartTime, ((float)restartTiming / 60f).ToString("0.##")));
            }

...
namespace Casevac
{
    public class JobDriver_CasevacRescue : JobDriver
    {
        ...
        public Pawn MainRescuee
        {
            get
            {
                var mainRescuee = (Takee.ParentHolder as Pawn_CarryTracker)?.pawn;
        ...
        public Pawn Taker
        {
            get
            {
                return (Takee.ParentHolder as Pawn_CarryTracker)?.pawn;
            }
        }
        ...
        public override bool TryMakePreToilReservations(bool errorOnFailed)
        {
            ...
            var otherRescuers = pawn.Map.mapPawns.SpawnedPawnsInFaction(Faction.OfPlayer).Where(x => x.CurJobDef == CP_DefOf.CP_CasevacRescue
                    && x.CurJob.targetA.Thing == TargetA.Thing && x.CurJob.targetB.Thing != null);

            foreach (var otherPawn in otherRescuers)
            {
        ...
        private void ReleaseAndMakeOtherReserveBed()
        {
            ...
            var mainRescuee = MainRescuee;
            if (mainRescuee != null && Takee.CurrentBed() != DropBed)
            {
        ...
        public override IEnumerable<Toil> MakeNewToils()
        {
            ...
            {
                if (DropBed.ForPrisoners != Takee.IsPrisoner)
                {
                    return true;
                }
                return false;
            ...
            toil.tickAction = delegate
            {
                if (TargetA.Thing.ParentHolder is Pawn_CarryTracker pawn_CarryTracker)
                {
                    if (!pawn.pather.MovingNow)
                    {
                        ...
                        {
                            if (!Pawn_PathFollower_StopDead.lastPathTicks.TryGetValue(pawn, out var value) || value != Find.TickManager.TicksGame)
                            {
            ...
            yield return new Toil
            {
                initAction = delegate ()
                {
                    foreach (var rescuee in Rescuers)
                    {
            ...
            toil4.initAction = delegate
            {
                ...
                foreach (var rescuee in Rescuers)
                {
                    ...
                    foreach (var rescuee2 in Rescuers)
                    {
                        if (rescuee != rescuee2)
                        {
